<template>
  <div class="max-w-xs mx-auto md:m-0 sm:flex-none">
    <div class="relative z-30 flex flex-row font-bold">
      <img
        :src="project.logo === '%avatar%' ? avatarUrl : project.logo"
        alt="Logo"
      ...
      <h2 class="text-3xl w-full leading-10 mb-2">
        ...
        <a class="project-link" :href="project.links.ext">
          {{ project.title }}
          <ArrowTopRightOnSquareIcon v-if="project.links.ext" class="link-icon inline -mt-1 w-6 h-6 opacity-0 transition-opacity duration-300 ease-in-out" />
        </a>
      </h2>
    </div>
...
export default {
  name: 'Project',
  components: { ProjectThumbnail, Tag, ArrowTopRightOnSquareIcon },
  props: {
    project: {
      type: Object,
      default: () => ({})
