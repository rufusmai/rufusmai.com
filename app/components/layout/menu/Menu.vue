<template>
  ...
  >
    <button
      class="absolute sm:hidden inset-0 h-full w-full transition duration-300 ease-in-out cursor-default"
      :class="menuOpened ? 'bg-black/25' : ''"
    ...
    >
      ...
      <Navbar
        ...
        class="mt-2"
        @update:menu-opened="toggleMenu"
        @pwa-installed="$emit('pwa-installed')"
      />
...
import LanguageChoose from './chooser/LanguageChoose.vue'
import ThemeChoose from './chooser/ThemeChoose.vue'
import SocialMedia from './SocialMedia.vue'
import Navbar from './navbar/Navbar.vue'

export default {
  name: 'SiteMenu',
  components: { Navbar, SocialMedia, LanguageChoose, ThemeChoose, XMarkIcon },
  mixins: [menuToggle],
  props: {
    installPrompt: {
      type: Object,
