  ...
  <div class="content-wrapper block h-full mx-auto w-full">
    <div class="pb-2 space-y-8 md:space-y-0 md:space-x-8 md:flex flex-row flex-no-wrap md:overflow-x-auto scroll">
      <div v-for="(project, id) in projects" :key="id" class="max-w-xs mx-auto md:m-0 sm:flex-none">
        <div class="flex flex-row font-bold">
          <img :src="project.logo" alt="Logo" class="h-12 w-12 mr-3 bg-white bg-opacity-50 rounded-full">
          <h2 class="text-3xl leading-10" :class="project.title === 'Portfolio' ? 'mb-2' : 'mb-4'">
            <span v-if="project.title === 'Portfolio'" class="relative block mb-3">
              <span class="absolute block text-base font-medium text-gray-300 leading-none" style="margin-left: 1px">
                {{ $t('thisSite') }}
              </span>
            </span>
            {{ project.title }}
        ...
        <div class="flex flex-row justify-between space-x-2 w-full">
          <a
            v-if="Object.keys(project.links).includes('ext')"
          ...
          >
            {{ $t('openPage') }}
            <ExternalLinkIcon class="inline -mt-1" size="20" />
          </a>
          <nuxt-link
            v-if="Object.keys(project.links).includes('int')"
            :to="project.links.int"
            class="font-medium text-xl transition-colors duration-300 ease-in-out text-gray-100 hover:text-gray-300 focus:outline-none"
          >
            <HomeIcon class="inline -mt-1" size="20" />
            {{ $t('toHome') }}
          </nuxt-link>
          <a
          ...
          >
            ...
            <span class="px-2 py-1">
              <span class="xs:hidden">1GMG Login</span>
              <span class="hidden xs:inline">Login {{ $t('with') }} 1GMG</span>
            </span>
          </button>
        </div>
        ...
        <img
          ...
          width="320"
          height="180"
          class="rounded-lg mt-2 bg-gray-600 opacity-75"
        >
        ...
        <div class="mt-1 font-bold">
          <span
            ...
            :key="tagId"
            class="inline-block px-3 py-2 mt-2 mr-2 rounded-full bg-gray-500 bg-opacity-50"
          >
            {{ tag }}
          </span>
        </div>
    ...
    <div class="text-center md:text-left">
      ...
      <div v-else class="gh-wrapper mt-4 pb-4 space-y-8 md:space-y-0 md:space-x-4 md:flex flex-row flex-no-wrap md:overflow-x-auto scroll">
        <div v-for="repo in githubRepos" :key="repo.id" class="mx-auto max-w-sm md:m-0 bg-gray-400 flex-none flex flex-col bg-opacity-50 rounded-lg px-4 py-2">
          <div class="mb-2">
          ...
          <div class="mt-auto space-x-3 font-medium">
      ...
      <div v-if="githubRepos.length > 0" class="live-indicator text-gray-400 md:mt-2">
        <span class="inline-block bg-green-300 w-2 h-2 rounded-full" />
        {{ $t('liveFrom') }} <fai :icon="['fab', 'github']" /> API
      </div>
    </div>
  </div>
</template>
